...
var m_layer_municipios = create_layer_kml_base('Municipios', '', './kml/puebla.kml', .3, true);
...
notification.innerHTML = '<button title="Desarrollo"><i class="glyphicon glyphicon glyphicon-cog"></i></button>';

notification.addEventListener('click', function () {
	m_notification.show('SECRETARÍA DE MEDIO AMBIENTE, DESARROLLO SUSTENTABLE Y ORDENAMIENTO TERRITORIAL', 5000);
});
...
var m_view = new ol.View({
		...
		zoom: 8.3,
		minZoom: 6							//Entre mas bajo se aleja mas
});
...
var m_map = new ol.Map({	
	controls: ol.control.defaults().extend([
						...
						m_notification, m_control, 
						new ol.control.LayerSwitcher({							//Control de capas
							tipLabel: 'Capas'
						})
	]),
  target: 'map',
...
//-------------------------------------------------------------------------------
$(function(){
	make_transaction(mUrl_api + 'api.php?tipo_solicitud=listado_runs', 'fecha=' + '20240131', 
									 list_runs, showDialog_Error);
	make_transaction(mUrl_api + 'api.php?tipo_solicitud=cabeceras', 'fecha=' + '20240131',
									 list_cabeceras, showDialog_Error);
	make_transaction(mUrl_api + 'api.php?tipo_solicitud=estaciones', 'fecha=' + '20240131',
									 list_estaciones, showDialog_Error);
});
...
//-------------------------------------------------------------------------------
var make_transaction = function(murl, mdata, function_on_success, function_on_error){
	$.ajax({url: murl,
		type: 'POST',
		data: mdata,
...
var crea_mask = function() {
    ...
    selectedOption.textContent = recortado.replace(/(\d{4})(\d{2})(\d{2})/, '$3-$2-$1');
}
...
var procesa_var = function(){
	...
	make_transaction(mUrl_api + 'api.php?tipo_solicitud=listado_var', str_dat, list_var, showDialog_Error);
};
...
m_view.on("propertychange", function(e){
	if(e.key == 'resolution'){						//Cuando cambia el zoom
...
function show_feature(tipo, dir_dat, show_dialog){									
	...
	contenDialog.append('<a href="#" onclick="downloadFileCSV();">'+'Descargar datos'+'</a>');
...
function set_chart_chem(str_file, contenDialog, show_dialog){	
	...
	$.ajax({
		...
		success : function (data){
			...
			if(show_dialog){
				set_canva(contenDialog, djson['CO'], 'line', str_file, 'Monóxido de Carbono', 'ppm', 'rgb(120, 40, 31)');
				set_canva(contenDialog, djson['NO2'], 'line', str_file, 'Dióxido de Nitrógeno', 'ppb', 'rgb(74, 35, 90)');
				set_canva(contenDialog, djson['O3'], 'line', str_file, 'Ozono', 'ppb', 'rgb(14, 98, 81)');
				set_canva(contenDialog, djson['SO2'], 'line', str_file, 'Dióxido de Azufre', 'ppb', 'rgb(21, 67, 96)');
				set_canva(contenDialog, djson['PM10'], 'line', str_file, 'Partículas PM 10', 'µg/m³', 'rgb(125, 102, 8)');
				set_canva(contenDialog, djson['PM25'], 'line', str_file, 'Partículas PM 2.5', 'µg/m³', 'rgb(77, 86, 86)');		
			}
			else{
				downloadFileCSV();
			}						
...
function setLabel(str_file, hs){
	...
	var fecha_loc = pad(f.getDate(), 2)+'/'+pad(f.getMonth()+1, 2)+'/'+f.getFullYear()+' '+pad(f.getHours(), 2)+'hs';
...
function show_datos(datos) {
	...
	features.sort(function(a, b){ return a.get('clave') > b.get('clave') ? 1 : b.get('clave') > a.get('clave') ? -1 : 0 });
	...
	texthtml.append('<div>');
	texthtml.append('<div style="display: inline-block; width: 20%; vertical-align: top; background-color: #f2f2f2; padding: 4px; box-sizing: border-box; text-align: center;" >Clave</div>');
	texthtml.append('<div style="display: inline-block; width: 60%; vertical-align: top; background-color: #f2f2f2; padding: 4px; box-sizing: border-box; text-align: center;" >Municipio</div>');
	texthtml.append('<div style="display: inline-block; width: 20%;  vertical-align: top; background-color: #f2f2f2; padding: 4px; box-sizing: border-box; text-align: center;" >Opciones</div>');
	texthtml.append('</div>');
	...
	for(var i = 0; i < features.length; i++){
		...
		//texthtml.append('<tr>');
		texthtml.append('<div <div style="display: inline-block; width: 20%; vertical-align: top; background-color: #f9f9f9; padding: 1px; box-sizing: border-box; border: 1px solid #fff;" >' + feature.get('clave') + '</div>');
		texthtml.append('<div style="display: inline-block; width: 60%; vertical-align: top; background-color: #f9f9f9; padding: 1px; box-sizing: border-box; border: 1px solid #fff;" >' + feature.get('nombre') + '</div>');
		
		var str_link = '<a href="#" onclick="downladCSV(\'' + feature.get('clave') + '\');">' + '<i class="glyphicon glyphicon-file"></i>' + '</a>';
		texthtml.append('<div style="display: inline-block; width: 20%; vertical-align: top; background-color: #f9f9f9; padding: 1px; box-sizing: border-box; border: 1px solid #fff; text-align: center;" >' + str_link + '</div>');
		//texthtml.append('</tr>');
	}
