...
async function loadGradientDataFromCSV(path, variable, units, title) {
  ...
  // Ya no usamos canvas vertical, ahora creamos gradiente CSS horizontal
  const cssGradient = `linear-gradient(to right, ${colors.join(', ')})`;
  
  // Actualizar la nueva leyenda horizontal
  updateLegend(title, cssGradient, min, max, units || "");
  
  // Mantener compatibilidad con el sistema de filtrado existente
  // (el canvas ya no se usa visualmente pero mantenemos la lógica)
  if (canvas) {
